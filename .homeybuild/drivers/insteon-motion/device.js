...
class InsteonMotionDevice extends Homey.Device {

  async onInit() {
    this.log('Insteon Motion Sensor device initialized');
  ...
  async onAdded() {
    this.log('Insteon Motion Sensor device has been added');
  }

  async onSettings({ oldSettings, newSettings, changedKeys }) {
    const app = this.homey.app;
    app.debugLog(`Motion sensor settings changed: ${this.getName()}`);
    ...
    if (changedKeys.includes('deviceID')) {
      ...
      if (this.deviceID && this.deviceID !== '') {
        this.log('Motion sensor configured with ID:', this.deviceID);
        app.debugLog(`  Registering motion sensor with Insteon ID: ${this.deviceID}`);
        ...
        app.debugLog(`  Motion sensor marked as available`);
      } else {
  ...
  async onDeleted() {
    this.log('Insteon Motion Sensor device has been deleted');
    const app = this.homey.app;
    app.unregisterDevice(this);
  }
...
module.exports = InsteonMotionDevice;
